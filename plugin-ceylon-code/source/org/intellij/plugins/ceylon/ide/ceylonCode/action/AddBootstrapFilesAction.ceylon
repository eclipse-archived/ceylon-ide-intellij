...
shared class AddBootstrapFilesAction() extends AnAction() {
    ...
    shared actual void actionPerformed(AnActionEvent evt) {
        assert(exists mod = DataKeys.\iMODULE_CONTEXT.getData(evt.dataContext));
        ...
        if (exists project = projects.getProject(mod),
            exists tcInvoker = Extensions.getExtensions(ITypeCheckerInvoker.\iEP_NAME).get(0),
            exists version = JOptionPane.showInputDialog(null,
                "Select a Ceylon version", messages.bootstrap.title,
                JOptionPane.\iQUESTION_MESSAGE, null, createJavaStringArray(versions), versions.first)) {
            ...
            while (!success) {
                ...
                case (is Boolean) {
                    ...
                    value result = Messages.showOkCancelDialog(
                        messages.bootstrap.filesExist,
                        messages.bootstrap.title,
                        Messages.questionIcon
                    );
                    
                    if (result == Messages.\iOK) {
                        force = true;
                    } else {
                        return;
                    }
    ...
    shared actual void update(AnActionEvent evt) {
        ...
        if (exists mod = DataKeys.\iMODULE_CONTEXT.getData(evt.dataContext)) {
            pres.enabled = true;
            pres.visible = true;
        } else {
            pres.enabled = false;
